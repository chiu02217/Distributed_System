...
class AFSClient(IAFSClient):
    def __init__(self, server_address='localhost:8000', cache_dir='/tmp/afs'):
        ...
        self.open_files = {}
        print(f"[AFS] Connected to server at {server_address}, cache dir: {self.cache_dir}")
    ...
    def open_file(self, filename):
        try:
            ...
            print(f"[AFS] Exception during open_file: {e}")
            return None
    ...
    def create_file(self, filename):
        try:
            ...
            if response.error:
                print(f"[AFS] File creating error: {response.error}")
                return None
            ...
            print(f"[AFS] Created file: {filename} with handle {response.handle}")
            return response.handle
        
        except Exception as e:
            print(f"[AFS] Exception during create_file: {e}")
            return None
    ...
    def read_file(self, handle):
        if handle not in self.open_files:
            print(f"[AFS] Error reading file: {handle}")
            return None
        ...
        try:
            ...
            print(f"[AFS] Error during read_file: {e}")
            return None
    ...
    def close_file(self, handle):
        ...
        if response.success:
            ...
            print(f"[AFS] Error closing file: {response.error}")
            return False
    ...
    def list_files(self):
        try:
            ...
            if response.error:
                print(f"[AFS] Error listing files: {response.error}")
                return []
            
            return list(response.filenames)
        
        except Exception as e:
            print(f"[AFS] Exception during list_files: {e}")
            return []