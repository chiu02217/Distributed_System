...
start "AFS Server" cmd /k python -m src.afs.server.afs_server
timeout /t 2 > nul
start "Coordinator" cmd /k python -m src.afs.coordinator.coordinator_server
timeout /t 2 > nul
start "Worker 1" cmd /k python -m src.app.worker.worker_client.worker_client worker-1
start "Worker 2" cmd /k python -m src.app.worker.worker_client.worker_client worker-2
start "Worker 3" cmd /k python -m src.app.worker.worker_client.worker_client worker-3
...
start "Worker 2 (Restored)" cmd /k python -m src.app.worker.worker_client.worker_client worker-2
...
wmic process where "name='python.exe' and commandline like '%%coordinator_server%%'" call terminate > nul
...
start "Coordinator (Restored)" cmd /k python -m src.afs.coordinator.coordinator_server
