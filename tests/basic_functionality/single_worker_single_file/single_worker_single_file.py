...
def test_single_worker_single_file():
    try:
        ...
        assert wait_for_log_in_queue(log_queue, "[AFS Server] Updated file: primes.txt", timeout=5), "[TEST] AFS server did not write to primes.txt"
        assert wait_for_log_in_queue(log_queue, "[Worker worker-1] Submitted", timeout=5), "[TEST] Worker 1 did not submit primes"
        assert wait_for_log_in_queue(log_queue, "[Worker worker-1] No more tasks available", timeout=5), "[TEST] Worker 1 did exit"
