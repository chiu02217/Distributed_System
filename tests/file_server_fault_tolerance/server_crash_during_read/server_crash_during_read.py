...
def test_server_crash_during_read():
    try:
        log_queue = Queue()
        ...
        assert wait_for_log_in_queue(log_queue, "[AFS Server] Updated", timeout=5), "[TEST] AFS server did not write to primes.txt"

        assert wait_for_log_in_queue(log_queue, "[AFS Server] Reading file: input_dataset_big.txt", timeout=10), "[TEST] AFS server did not start reading input_dataset_big.txt"
        
        #crash server
        print("[TEST] Crashing server...")
        server.kill()

        assert wait_for_log_in_queue(log_queue, "[Worker worker-1] Error: Could not read file", timeout=20), "[TEST] Worker did not detect server crash"
